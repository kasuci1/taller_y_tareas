...
void eliminator(int valor){
    ...
    remove("trabajadores.txt");                          ////proceso que escribe los datos actualizados en el archivo de entrada trabajadores.txt
    archivo=fopen("trabajadores.txt","w");                //apertura de archivo de entrada
...
void paramenter_changer(int valor, int cont){                       //parameter changer
    ...
    remove("trabajadores.txt");                          //borra archivo de entrada
    archivo=fopen("trabajadores.txt","w");              //lo prepara para el update
    for(i=0;i<cont;i++){                                    
        sprintf(temp,"%i",trab[i].cedula);
        fputs(temp,archivo);
        fputs(" ",archivo);
...
void direct_sort(int vector_matriz[],int cont,int indice_vec_sueldo[]){         //is another sort algorithm xD!
    ...
    for(actual=0;actual<cont;actual++){
        smallest=actual;
        for(j=actual+1;j<cont;j++){
...
void modulo_consultar(){                                   //modulo de consulta
    int i=0,j,valor,bandera=1,cont=0,aux=0,desicion;
    FILE *archivo;
    ...
    deparchivo=fopen("departamento.txt","a");
    if(deparchivo==NULL){
        ...
        case 3:
            ...
            for(i=0;i<cont;i++){
                if(strcmp(temp,trab[i].cargo)==0){                                                   
                    ...
                    fputs(tempaux,cargochivo);
                    fputs(" ",cargochivo);
                    vaciar(tempaux);                //estructura que actualiza cargo.txt de salida
                    strcpy(tempaux,trab[i].cargo);
                    fputs(tempaux,cargochivo);
                    fputs(" ",cargochivo);
                    vaciar(tempaux);
